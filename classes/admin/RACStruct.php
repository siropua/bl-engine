...
class RACStruct{
	...
	function getSections($force_refresh = false){
		...
		$dirs = glob($this->_basePath.'/*', GLOB_ONLYDIR);
	...
	function getModules($section, $force_refresh = false)
	{
		...
		$dirs = glob($this->_basePath.'/'.$section.'/*', GLOB_ONLYDIR);
		if($dirs && is_array($dirs))
		foreach($dirs as $s)
		{

			$moduleName = basename($s);

			###### cvs fix #########
			if($moduleName == 'CVS') continue;
...
class RACMenuWorker{
	...
	public function parseModules($forceReload = false){
		...
		$globalURL = ROOT_URL.ENGINE_FOLDER.'/admin/modules/';
